...
{
	// Сохраняем ссылки на дескрипторы 
	mhCpuSrv = hCpuSrv;
	mhGpuSrv = hGpuSrv;
	mhCpuDsv = hCpuDsv;
...
void ShadowMap::OnResize(UINT newWidth, UINT newHeight)
{
	if ((mWidth != newWidth) || (mHeight != newHeight))
	{
		...
		// Для новых ресурсов нам нужны новые дескрипторы
		BuildDescriptors();
	}
}
...
{
	// Создаём SRV для ресурса, чтобы мы могли сэмплировать карту теней в программе шейдера
	...
	// Создаём DSV для ресурса, чтобы была возможность выполнить рендеринг на карту теней
